...
import { Bus, MapPin, Clock, Edit, Save, X, Trash2, Video } from "lucide-react";
    ...
    <>
        ...
        <DialogContent className="sm:max-w-[500px] bg-white">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <input
                type="text"
                value={editingBus.erpId}
                onChange={(e) => setEditingBus({ ...editingBus, erpId: e.target.value })}
                className="col-span-3 border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300"
      ...
      <Dialog open={cctvModalOpen} onOpenChange={setCctvModalOpen}>
        <DialogContent className="sm:max-w-[800px] bg-white text-black">
          ...
          <div className="py-4">
            <CCTVFeed
              deviceId={overview.status.id}
              vehicleLabel={busData?.plateNumber || overview.plate_number || overview.status.id}
            />
          </div>
        </DialogContent>
      </Dialog>
